...
class APIServer(FastAPI):
    ...
    def configure_routes(self) -> None:
        ...
        _ = gr.mount_gradio_app(
            self,
            blocks=pages.stats.build_page(),
            path=f"/content{pages.stats.PATH}",
        )

        _ = gr.mount_gradio_app(
            self,
            blocks=pages.waveform.build_page(),
            path=f"/content{pages.waveform.PATH}",
        )
        ...
        @self.get("/waveform")
        async def kb_redirect() -> FileResponse:
            return FileResponse(os.path.join(STATIC_DIR, "waveform.html"))
