...
class StrategyProposerAgent(BaseAgent):
    def execute(self, task: Dict[str, Any]) -> AgentResponse:
        prompt = self.format_prompt(task)
        system_prompt = (
            "You propose executable optimization strategies in YAML. "
            "Return a strategy block with algorithm, population, generations, and execution settings."
        )
