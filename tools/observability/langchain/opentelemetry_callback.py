...
def _create_span_attr(
    span: Span, span_attrs: Dict[str, Any], span_end: bool = False
) -> None:
    ...
    if span_end:
        ...
        span_attrs.update(
            {"runtime_info": runtime_info, "system_metrics": system_metrics}
        )
        ...
        **kwargs: Any,
        ...
        try:
            ...
            span = self._tracer.start_span(
                f"langchain.llm.{class_name}", context=parent_ctx
            )
        ...
        **kwargs: Any,
        ...
        try:
            ...
            if parent_run_id not in self._event_map:
                ...
                parent_ctx = self._event_map[parent_run_id]["span"].context
            span = self._tracer.start_span(
                f"langchain.chat_model.{class_name}", context=parent_ctx
            )
            ctx = set_span_in_context(span)
            token = attach(ctx)
            self._event_map[run_id] = {}
    ...
    def on_llm_new_token(self, token: str, chunk, run_id: UUID, **kwargs: Any) -> None:
        ...
        if run_id in self._event_map:
            ...
            if self.llm_tokens == 1:
                self._event_map[run_id]["first_token_time"] = (
                    token_time - self._event_map[run_id]["start_time"]
                )
        ...
        try:
            ...
            if run_id in self._event_map:
                ...
                if self.llm_tokens:
                    ...
                    self._event_map[run_id]["first_token_time"] = (
                        llm_end_time - self._event_map[run_id]["start_time"]
                    )
                ...
                span_attrs = {
                    ...
                    "first_token_time_ms": self._event_map[run_id][
                        "first_token_time"
                    ].total_seconds()
                    * 1000,
                }
        ...
        **kwargs: Any,
        ...
        try:
            ...
            if not parent_run_id:
                ...
                parent_ctx = self._event_map[parent_run_id]["span"].context
            span = self._tracer.start_span(
                f"langchain.chain.{class_name}", context=parent_ctx
            )
            ctx = set_span_in_context(span)
            token = attach(ctx)
            self._event_map[run_id] = {}
        ...
        **kwargs: Any,
        ...
        try:
            ...
            if not parent_run_id:
                ...
                parent_ctx = self._event_map[parent_run_id]["span"].context
            span = self._tracer.start_span(
                f"langchain.tool.{class_name}", context=parent_ctx
            )
            ctx = set_span_in_context(span)
            token = attach(ctx)
            self._event_map[run_id] = {}
        ...
        **kwargs: Any,
        ...
        try:
            ...
            if not parent_run_id:
                ...
                parent_ctx = self._event_map[parent_run_id]["span"].context
            span = self._tracer.start_span(
                f"langchain.retriever.{class_name}", context=parent_ctx
            )
            ctx = set_span_in_context(span)
            token = attach(ctx)
            self._event_map[run_id] = {}
