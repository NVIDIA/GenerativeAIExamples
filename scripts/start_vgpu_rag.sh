...
show_final_status() {
    ...
    docker ps --format "table {{.Names}}\t{{.Status}}" | grep -E "(milvus|ingestor|rag|nim)"
    echo ""
    ...
    if [ "$ENABLE_BOOTSTRAP" = "true" ]; then
        print_info "vGPU Collections Created:"
        echo "  • vgpu_baseline - Core NVIDIA vGPU documentation"
        echo "  • vgpu_hypervisor - ESXi, VMware, Citrix documentation"
        echo "  • vgpu_cost_efficiency - Cost optimization guides"
        echo "  • vgpu_performance - Performance benchmarks"
        echo "  • vgpu_a40, vgpu_l40s, vgpu_l4 - GPU-specific docs"
        echo "  • vgpu_sizing - VM sizing guides"
        echo ""
        print_info "To add vGPU documentation:"
        echo "  1. Place PDF files in: $VGPU_DOCS_PATH/<collection_subdir>/"
        echo "  2. Restart bootstrap: docker compose -f deploy/compose/docker-compose-bootstrap.yaml up"
    fi
    
    echo ""
    print_info "To stop all services:"
    echo "  ./scripts/stop_vgpu_rag.sh"
}
